export function renderTopNav({ currentChapter, progress = 0 }) {
  const container = document.createElement("header");
  container.className =
    "fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-3 bg-[#0B1120]/90 backdrop-blur-sm border-b border-terminal-green/30";
  ...
  container.innerHTML = `
    ...
    <div class="flex items-center space-x-4">
      <div class="flex items-center space-x-2">
        <span class="text-terminal-green/50 text-[10px] tracking-widest">PROGRESS</span>
        <div class="w-24 h-1.5 bg-black border border-terminal-green/30 relative">
          <div class="h-full bg-terminal-green transition-all duration-500" style="width: ${progress * 100}%"></div>
        </div>
      </div>
  ...
  const openModal = (title, content) => {
    ...
    modal.className =
      "fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm opacity-0 transition-opacity duration-300";
    modal.innerHTML = `
